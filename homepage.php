...
if(!$_SESSION['userId']) {
	...
	$sql = "SELECT *, price * quantity AS total FROM  orders WHERE user_id = '".$_SESSION['userId']."' ORDER BY order_id DESC";

 	$adminsql = "SELECT * , price * quantity AS total FROM orders ORDER BY order_id DESC";

	$result = $connect->query($sql) or die('Error, failed'); 	

	$uresult = $connect->query($query) or die('Error, failed'); 

	$aresult = $connect->query($adminsql) or die('Error, failed');
