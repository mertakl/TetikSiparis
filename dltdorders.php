...
if(!$_SESSION['userId']) {
	...
	$query = "SELECT is_admin FROM users WHERE user_id = '".$_SESSION['userId']."'";
 	$adminsql = "SELECT * FROM deleted_orders ORDER BY order_id DESC";	
	...
	<div class="table-responsive">  
	  <table class="table table-hover table-bordered" id = "tableExport">
		...
		<tbody>
		   ...
		   <?php 
				if($userdata['is_admin'] == 1){ //Only admin will see deleted ones
					foreach( $aresult as $data ) // using foreach  to display each element of array
						{
							...
							echo "<tr><td>".$data['order_situation']."</td>
									  ...
									  <td>".$data['username']."</td>
									  <td>".$data['date']."</td>
									  <td><a href=\"dltorder.php?id=".$data['order_id']."\"><button class='btn btn-danger'>Sil</button></a>  <!--Permanently remove data-->
									  </td>
								   </tr>";
